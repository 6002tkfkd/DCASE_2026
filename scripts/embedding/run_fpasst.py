#!/usr/bin/env python3
"""
fPaSST embedding extraction.

--mode all  : 3 ckpt × 7 seq × 7 chunk = 147 combos on BSD10k-v1.2
--mode 35k  : top 1 combo (ensemble_search 결과) on BSD35k-CS
                1. fpasst_weak_seq_meanmax_chunk_maxstd  (8/19)

Usage:
    python scripts/embedding/run_fpasst.py --mode all
    python scripts/embedding/run_fpasst.py --mode 35k
    python scripts/embedding/run_fpasst.py --mode all --skip-done
    python scripts/embedding/run_fpasst.py --mode all --dry-run
"""
import argparse
import os
import subprocess
...
FPASST_REPO_DIR = "visualize/3fpasst_and_atst_code/PretrainedSED"
EMBED_ROOT      = os.path.join(PROJECT_ROOT, "visualize", "embedding", "fpasst_embedding")

CHECKPOINTS = {
    "strong1": "fpasst_strong_1",
    "im":      "fpasst_im",
    "weak":    "fpasst_weak",
}
...
TOP_COMBOS = [
    ("weak", "fpasst_weak", "mean+max", "max+std"),
]
...
def combo_name(ckpt_name, seq_pool, chunk_pool):
    return f"fpasst_{ckpt_name}_seq_{pool_name(seq_pool)}_chunk_{pool_name(chunk_pool)}"
...
def build_cmds(ckpt_name, ckpt_file, seq_pool, chunk_pool, datasets, overwrite=False):
    ...
    for ds_name, ds_short in datasets.items():
        ...
        cmd = [
            sys.executable,
            "visualize/extract_fpasst_embeddings.py",
            "--fpasst-repo-dir", FPASST_REPO_DIR,
...
def main():
    parser = argparse.ArgumentParser(description="fPaSST embedding extraction")
