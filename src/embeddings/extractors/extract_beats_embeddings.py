...
def load_beats_model(beats_code_dir, checkpoint_path, device):
    sys.path.insert(0, os.path.abspath(beats_code_dir))
...
def make_chunks(waveform, chunk_samples, hop_samples):
    ...
    for start in starts:
        ...
        valid_len = chunk.numel()
        if valid_len < chunk_samples:
            chunk = torch.nn.functional.pad(chunk, (0, chunk_samples - valid_len))
...
def extract_dataset(model, dataset_name, args, device):
    ...
    for _, row in metadata.iterrows():
        ...
        try:
            ...
            chunks, padding_mask = make_chunks(waveform, chunk_samples, hop_samples)
            chunks = chunks.to(device)
            padding_mask = padding_mask.to(device)

            # bypass classification head to get transformer hidden states (768-dim)
            ...
            try:
                for start in range(0, chunks.shape[0], args.batch_size):
                    ...
                    batch_mask = padding_mask[start:start + args.batch_size]
                    with torch.no_grad():
                        features = model.extract_features(batch, padding_mask=batch_mask)[0]
...
def main():
    parser = argparse.ArgumentParser(description="Extract BEATs audio embeddings as .npy files.")
    parser.add_argument("--beats-code-dir", default="visualize/beats/unilm/beats")
    parser.add_argument(
        "--checkpoint",
        default="visualize/beats/unilm/checkpoint/BEATs_iter3_plus_AS2M.pt",
    )
    parser.add_argument("--datasets", default=None)
    parser.add_argument("--output-dir", default=None)
    parser.add_argument("--output-subdir", default="beats_audio_embeddings")
    ...
    parser.add_argument(
        ...
        help="Pooling over BEATs sequence frames inside each chunk.",
    )
    parser.add_argument(
        "--chunk-pooling",
        default="mean",
    ...
    model = load_beats_model(args.beats_code_dir, args.checkpoint, device)
