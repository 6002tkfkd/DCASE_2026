...
def load_fpasst_model(repo_dir, checkpoint, device):
    repo_dir = os.path.abspath(repo_dir)
    sys.path.insert(0, repo_dir)

    import config as fpasst_config

    fpasst_config.RESOURCES_FOLDER = os.path.join(repo_dir, "resources")
    os.makedirs(fpasst_config.RESOURCES_FOLDER, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(description="Extract fPaSST audio embeddings as .npy files.")
    parser.add_argument("--fpasst-repo-dir", default="visualize/3fpasst_and_atst_code/PretrainedSED")
    parser.add_argument("--checkpoint", default="fpasst_strong_1")
    parser.add_argument("--datasets", default=None)
    parser.add_argument("--output-dir", default=None)
    parser.add_argument("--output-subdir", default="fpasst_audio_embeddings")
    ...
    parser.add_argument(
        ...
        help="Pooling over fPaSST sequence frames inside each chunk.",
    )
    parser.add_argument(
        "--chunk-pooling",
        default="mean",
    ...
    model = load_fpasst_model(args.fpasst_repo_dir, args.checkpoint, device)
