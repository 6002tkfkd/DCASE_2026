...
def load_passt_model(arch, device):
    from hear21passt.base import get_basic_model, get_model_passt
    from hear21passt.models import passt as passt_models

    with contextlib.redirect_stdout(io.StringIO()):
        model = get_basic_model(mode="embed_only")
        try:
            model.net = get_model_passt(arch=arch, n_classes=527)
        except RuntimeError:
            model_func = getattr(passt_models, arch)
            model.net = model_func(pretrained=True)
    model.eval()
    model.to(device)
    return model
...
def main():
    parser = argparse.ArgumentParser(description="Extract PaSST audio embeddings as .npy files.")
    parser.add_argument(
        "--arch",
        ...
        choices=[
            ...
            "passt_s_f128_20sec_p16_s10_ap474_swa",
        ],
        help="PaSST model architecture to load.",
    )
    parser.add_argument("--datasets", default=None)
    parser.add_argument("--output-dir", default=None)
    parser.add_argument("--output-subdir", default="passt_audio_embeddings")
    ...
    model = load_passt_model(args.arch, device)
